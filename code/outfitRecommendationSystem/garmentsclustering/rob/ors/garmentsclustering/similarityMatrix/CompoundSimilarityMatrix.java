...
public class CompoundSimilarityMatrix
{
	...
	public CompoundSimilarityMatrix(int[] garmentsIds, final CompoundSimilarityFunctionBuilder similarityFunctionBuilder)
	{	
		this(garmentsIds,similarityFunctionBuilder,-1);
	}
	
	
	/**
	 * If oneRowMatrixGarmentId is > 0, copute the similarity row of the garment with id oneRowMatrixGarmentId. This id must be in the array garmetsIds
	 * @param garmentsIds
	 * @param similarityFunctionBuilder
	 * @param oneRowMatrix
	 */
	public CompoundSimilarityMatrix(int[] garmentsIds, final CompoundSimilarityFunctionBuilder similarityFunctionBuilder, int oneRowMatrixGarmentId)
	{	
		...
		{	
			if(filteredGarmentsIds.add(garmentsIds[i])) 
			{
		...
		if(oneRowMatrixGarmentId>0)
		{
			SimilarityFunction similarityFunction = similarityFunctionBuilder.build();
			similarityFunction.loadImages(garmentsIds);
			int[] slices = new int[similarityFunctionBuilder.getSimilarityFunctionBuilders().size()];
			for(int i=0;i<similarityFunctionBuilder.getSimilarityFunctionBuilders().size();i++)slices[i]=i;
			garmentsIds = Utils.toArray(filteredGarmentsIds);
			for(int i=0; i<filteredGarmentsIds.size(); i++)
			{
				
				int iid1 = garmentsIds[i];
				int ipos1 = garmentIDRowNumber.get(iid1);
				int iid2 = oneRowMatrixGarmentId;
				int ipos2 = garmentIDRowNumber.get(oneRowMatrixGarmentId);				
				double sim[] = ((CompoundSimilarityFunction) similarityFunction).getSimilarityComponents(iid1, iid2,slices);				
				for(int s:slices)
				{
					similarityMatrix.set(s,ipos1, ipos2,sim[s]);
					similarityMatrix.set(s,ipos2, ipos1,sim[s]);
				}	

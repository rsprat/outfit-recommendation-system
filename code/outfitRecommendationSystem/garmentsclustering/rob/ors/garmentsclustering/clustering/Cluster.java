...
public class Cluster {
	...
	public static void createRandomAbstractGarments()
	{
		...
		clearAbstractionDB();
		Random r = new Random();
		int numAGarmentsPerCategorie = 3;
		 List<CategoryI> cats = DBConnection.session().createQuery("from CategoryI").list();
		for(Object aux :cats)
		{		
			DBConnection.transaction();
			CategoryI cat = (CategoryI) aux;
			Hibernate.initialize(cat.getAbstractGarments());
			List<AbstractGarmentI> aGarments = new LinkedList<AbstractGarmentI>();
			for(int i =0;i<numAGarmentsPerCategorie;i++)
			{
				AbstractGarmentI aGarment = new AbstractGarmentI(cat);
				DBConnection.session().save(aGarment);
				aGarments.add(aGarment);
			}			
			Iterator<ConcreteGarment> it  =cat.getGarments().iterator();
			while(it.hasNext())
			{
				ConcreteGarmentI garment = (ConcreteGarmentI) it.next();
				garment.setAbstractgarment(aGarments.get(r.nextInt(numAGarmentsPerCategorie)));			
				//DBConnection.session().saveOrUpdate(garment);
			}
			for(int i =0;i<numAGarmentsPerCategorie;i++)
			{
			
				DBConnection.session().saveOrUpdate(aGarments.get(i));
			}
		}

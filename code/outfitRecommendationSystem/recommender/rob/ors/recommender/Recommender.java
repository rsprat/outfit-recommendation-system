...
public class Recommender {
	...
	private static int MINIMUM_VIEWS = 100;
	private static int USER_PUNCTUATION = 3;
	private static Random random = new Random();
	private static float WU = 0f;
	private static float WF = 0f;
	private static float WC = 1.0f;
	/**
	 * 
	 * @param uid
	 * @param garmentId
	 ...
	 */
	public static Map<AbstractOutfit,Float> getOutfitsList(Integer uid, Integer garmentId)
	{
		...
		*/
		...
		List<Integer> abstractOutfitsIds = DBConnection.session().createSQLQuery("select distinct(abstract_outfit_id) from abstract_outfits_components comp WHERE  comp.abstract_garment_id="+garment.getAbstractGarment().getId()).list();
	...
	{
		ConcreteGarmentI garment = (ConcreteGarmentI) DBConnection.session().get(ConcreteGarmentI.class,garmentId);
		Map<AbstractOutfit,Float> outfitsScores = getOutfitsList(uid, garmentId);
	...
	public static float score(ConcreteOutfit outfit, Member member)
	{
		...
		return likesOverViews*lOVFactor*WC + followingLikedOverAll*WF + memberPunctuation*WU;
	}

...
public class MemberGetter extends Getter {
	...
	private static void fillMemberData(String[] data, Member member,boolean update, int deepness)
	{
		...
		if(!data[1].equals("UNKNWON"))
		{
			...
			{			
				ConcreteOutfit outfit = OutfitGetter.getOutfit(integer(sid),update,deepness);
		...
		if(!data[2].equals("UNKNWON"))
		{
			...
			{
				ConcreteOutfit s = OutfitGetter.getOutfit(integer(sid),update,deepness);
	...
	private static void fillMemberSummary(String[] summary, Member member)
	{

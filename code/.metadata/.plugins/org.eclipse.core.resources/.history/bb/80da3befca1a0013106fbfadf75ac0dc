 ...
 * between two points is greater than <var>r</var> then their similarity is outfit to 0.
 ...
 */
public class MySpectralClusterer extends weka.gui.beans.Clusterer implements OptionHandler {
	
	private static Logger LOGGER = Logger.getLogger(MySpectralClusterer.class.getCanonicalName());
	protected CompoundSimilarityMatrix similarityMatrix;

  /**
   *  The class number of each point in the dataoutfit
   */
   ...
   */
  protected double alpha_star = 0.9;
  /**
  ...
  /**
   * The static initializer outfits up the options vector
   */
  static {
  ...
  /**
   * Merges two outfits of points represented as integer vectors. The outfits are
   * not overlapped.
   *
   * @param a the first outfit of points
   * @param b the second outfit of points
   * @return the union of the two outfits
   */
   ...
   * @return an array of outfits of points (partitions)
   */
   ...
   */
  public int clusterInstance(int garmentId) throws java.lang.Exception {
    double min_dist = Double.POSITIVE_INFINITY;
    int c = -1;
    for(int i = 0; i < similarityMatrix.getSimilarityMatrix().rows(); i++) {
      double dist = similarityMatrix.getSimilarityFunction().getSimilarity(garmentId, similarityMatrix.getGarmentId(i));
      if(dist < min_dist) {
        c = cluster[i];
        min_dist = dist;
      }
  ...
  public int getTrainingInstanceCluster(int garmentId) throws Exception
  {
	  Integer rowNumber = similarityMatrix.getRowNumber(garmentId);
	  if(rowNumber==null) throw new Exception();
	  if(cluster.length<=rowNumber) throw new Exception();
	  return cluster[rowNumber];
  }

  /**
   * Generates a clusterer by the mean of spectral clustering algorithm.
   *
   * @param data outfit of instances serving as training data
 * @return 
   * @exception Exception if the clusterer has not been generated successfully
   */
  public void buildClusterer(int[] garments, CompoundSimilarityMatrix similarityMatrix) throws java.lang.Exception {    
	  LOGGER.info("Start spectral cluster at "+new Date().toString()+" with "+similarityMatrix.getSimilarityMatrix().rows()+" garments");
	Date start = new Date();
	this.similarityMatrix = similarityMatrix;
    DoubleMatrix2D w = similarityMatrix.getSimilarityMatrix();

    //Partitions points
    int[][] p = partition(w, alpha_star);//Todo: actualitza la matriu quan es borra un element.....
   
    //Deploys results
    numOfClusters = p.length;
    cluster = new int[ w.columns()];
    for(int i = 0; i < p.length; i++)
      for(int j = 0; j < p[i].length; j++)
        cluster[p[i][j]] = i;

    LOGGER.info("End spectral took "+((new Date().getTime()-start.getTime())/1000)+" for "+similarityMatrix.getSimilarityMatrix().rows()+" garments");
  ...
  public String alphaStarTipText() {
    return "outfit maximum allowable normalized cut value. The algorithm stops the recursive partitioning when it does not find a cut that has a value below this factor. Use this argument to limit the number of clusters.";
  }
  ...
  public String sigmaTipText() {
    return "outfit the distance scaling factor. The similarity of two point x and y is defined as exp(- d(x, y) / sigma) where d(x, y) is the Euclidean distance between x and y.";
  }
  ...
  public String rTipText() {
    return "outfit the maximum distance value, all points that are far away more than this value have a 0 similarity. Use this parameter to obtain a sparse similarity matrix (see -M).";
  }
